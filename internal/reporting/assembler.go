...
func BuildPrompt(input *ReportInput) string {
	...
	if isReport {
		...
		b.WriteString("When evaluating structured sessions against a plan target (e.g. '45 min at 200W'), use the power zone timeline to find the main block duration and average — not the AvgP(W) column, which is the session-wide average diluted by warm-up and cool-down. A Z1 block at the start and end of a structured ride is a planned component, not a deviation.\n\n")
		b.WriteString("The AvgHR column is also the session-wide average including warm-up and cool-down. Never cite it as HR compliance evidence for structured sessions — it will always read lower than actual working-block HR. To check an HR constraint: (1) find the main working block(s) in the power zone timeline by their start/end minutes, (2) read the HR zone timeline for those same minutes, (3) report the working-block HR average and peak zone. For example: if power shows two 15-min Z2 blocks at 10–25 min and 30–45 min, the relevant HR is what the HR zone timeline shows for those windows — not the session average.\n\n")
		b.WriteString("When a session prescription includes an explicit success criterion, treat it as the primary pass/fail gate. Conditional in-session adjustments in the plan (e.g. 'reduce power if HR exceeds X') are coaching guidance — use your judgment to decide whether failing to apply them represents a meaningful deviation or simply a harder-than-expected execution. The answer depends on context: a minor HR overshoot on an otherwise successful block is different from a session where the HR constraint was the whole point.\n\n")
	} else {
