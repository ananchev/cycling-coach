...
func BuildPrompt(input *ReportInput) string {
	...
	if isReport {
		...
		b.WriteString("When evaluating structured sessions against a plan target (e.g. '45 min at 200W'), use the power zone timeline to find the main block duration and average — not the session average power, which is diluted by warm-up and cool-down. A Z1 block at the start and end of a structured ride is a planned component, not a deviation. Similarly, do not use session average HR to check HR compliance targets — use the HR zone distribution and HR zone timeline, which show what actually happened during the working portion of the ride.\n\n")
